...
class MainLoop():
    ...
    def callback(self, message_rec):
        ...
        self.motor1.duty_cycle = self.microSec_to_duty(
            message_rec.data[0], pca.frequency)
        self.motor2.duty_cycle = self.microSec_to_duty(
            message_rec.data[1], pca.frequency)
        self.motor3.duty_cycle = self.microSec_to_duty(
            message_rec.data[2], pca.frequency)
        self.motor4.duty_cycle = self.microSec_to_duty(
            message_rec.data[3], pca.frequency)
        self.motor5.duty_cycle = self.microSec_to_duty(
            message_rec.data[4], pca.frequency)  # this was also 3, why?
        self.motor6.duty_cycle = self.microSec_to_duty(
            message_rec.data[5], pca.frequency)
        self.motor7.duty_cycle = self.microSec_to_duty(
            message_rec.data[6], pca.frequency)
        self.motor8.duty_cycle = self.microSec_to_duty(
